...
const newsThai = news.results.filter(function(elemant, index){
    return elemant.language === 'thai' && elemant.source_id === 'thaipbs'
}).map(function(elemant,index) {
    return({
        title:elemant.title,
        link: elemant.link,
        description: elemant.description,
        source_id: elemant.source_id,
        image_url: elemant.image_url
    })
})
